        ...
        void solve(NodeColors& output, GraphColoring& input, std::function<bool()> isTimeout, int seed) {
            ...
            */
            ...
            if (population.min_conflict == 0)
            {
                ...
                for(int i=0;i<test.num_vertex;i++)
                {
                    cerr << test.population_solution[population.min_conflict_index].solution[i+1] << " ";
                }
                cerr << endl;

                cerr << "conflict of solution 19: ";
                cerr << test.compute_conflict(test.population_solution[19].solution) << endl;
            ...
            for (int i = 0;i < input.nodeNum;i++)
            {
                output[i] = test.population_solution[population.min_conflict_index].solution[i+1];
            }
