...
void Hybrid_Evolution::hybrid_evolution_duet_1(long long int max_iter)
{
    ...
    Partition_Solution child(num_vertex, num_color);
    ...
    memset(&child.num_colors[0], 0, sizeof(child.num_colors[0]) * child.num_colors.size());
    ...
    cross_over(population_solution[p1], population_solution[p2], child.solution);

    // debug information;
    // cerr << "solution generated by cross over: ";
    // for(int i=0;i<input.nodeNum;i++)
    // {
    //     cerr << child.solution[i] << " ";
    // }
    // cerr << endl;
    ...
    tabu_search(child.solution, true, max_iter); // 仅仅需要对新形成的child进行禁忌搜索;

    // 由child的index1s构造出partition, index2s, color_num;
    child.construct_partition();
    ...
    population_solution[max_conflict_index] = child; // 将种群中冲突数最大的替换成child
