...
这篇文章提出了一种定制非支配排序基于偏随机键的遗传算法（customized non-dominated sorting based random-key genetic algorithm, NDS-BRKGA），它结合了两种经典的进化多目标优化方法：**偏随机键遗传算法 (Biased Random Key Genetic Algorithm, BRKGA) **和非支配排序遗传算法II（Non-dominated Sorting Genetic Algorithm II, NSGA-II）。这种算法针对BI-TTP的以下特点进行了定制：（i）利用已有的子问题求解器生成初始种群，结合子问题的近似最优解和随机解，保持种群的多样性；（ii）利用修复操作处理超过背包容量限制的不可行解，使算法只在可行解空间搜索；（iii）利用偏随机键编码处理混合变量类型，将排列和二进制向量映射为实数向量，便于应用传统的进化重组算子；（iv）利用非支配排序和拥挤距离进行生存选择，保证最终种群包含一组具有良好多样性的非支配解。文章还详细介绍了算法中各个步骤和参数的设计，并在不同的测试问题上进行了实验评估，证明了算法的有效性和稳健性。
...
非支配排序遗传算法II（Non-dominated Sorting Genetic Algorithm II, NSGA-II）是一种常用的多目标优化算法，用于解决多目标优化问题。

NSGA-II的核心思想是通过非支配排序和拥挤度距离来维护种群的多样性，以便更好地探索解空间中的Pareto前沿。Pareto前沿是指在多目标优化问题中，所有最优解组成的集合，其中一个解的改进必然会导致至少一个目标函数的恶化。

NSGA-II的算法流程如下：

1. 初始化种群。通常采用随机初始化的方式。

2. 针对种群中的每个个体，计算它们的非支配等级。

3. 基于非支配等级，进行拥挤度距离计算，以便维护多样性。

4. 使用非支配排序和拥挤度距离进行选择操作，生成下一代种群。

5. 对新种群进行变异和交叉操作。

6. 重复步骤2到5，直到达到预定的停止准则。

在NSGA-II中，非支配排序的核心思想是根据解的支配关系将种群分成若干层，每一层都是由一些非支配解组成的。拥挤度距离是一种衡量解在Pareto前沿中的分布程度的指标，用于维护种群的多样性。通过选择具有较高非支配等级和较大拥挤度距离的解，可以维护种群的多样性，并逐步收敛到Pareto前沿。

总的来说，NSGA-II是一种高效的多目标优化算法，适用于各种类型的多目标优化问题。它通过非支配排序和拥挤度距离来维护种群的多样性，以便更好地探索解空间中的Pareto前沿。
