 ...
 */
 ...
 /*
  * This function is modified from RecordBetterTour and original WriteTour.
  * I think it is unnecessary to record N->id into tmpTour.
  * However, to avoid errors caused by unfamiliarity with its code, I keep it.
  *
  * It only supports TSP.
  */
void WriteTour(std::vector<int>& v, std::vector<int>& tmpTour) {
	...
	tmpTour[0] = tmpTour[DimensionSaved];


	int j, n, Forward, a;

	n = DimensionSaved;
	for (i = 1; i < n && tmpTour[i] != MTSPDepot; i++);
	Forward = Asymmetric ||
		tmpTour[i < n ? i + 1 : 1] < tmpTour[i > 1 ? i - 1 : Dimension];
	for (j = 0; j < n; j++) {
		if ((a = tmpTour[i]) <= n)
			v[j] = a;
		if (Forward) {
