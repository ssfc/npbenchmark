...
LKMatrix::LKMatrix(vector<pair<float, float> >& coords, vector<int>& ids) {
	this->coords = coords;
	this->ids = ids;
	size = ids.size();
	...
	edgeDistances = vector<vector<float> >(size, vector<float>(size, 0));
...
float LKMatrix::getCurrentTourDistance() {
	int currentIndex = 0;
	float distance = 0;
	for (int i = 0; i < size; i++) {
...
void LKMatrix::LKMove(int tourStart) {
	...
	int nextV;
	int nextFromV;
	int lastPossibleNextV = 0;
	pair<int, int> broken_edge;
	float y_opt_length;
	float broken_edge_length;
	float g_opt_local;
...
vector<int> LKMatrix::getTourIds() {
	vector<int> resultTour;
	int current = 0;
	do {
		//		cout << ids[current] << endl;
		resultTour.push_back(ids[current]);
		current = tour[current];
	} while (current != 0);
	return resultTour;
}