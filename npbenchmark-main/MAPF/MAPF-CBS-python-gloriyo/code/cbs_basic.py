...
class CBSSolver(object):
    ...
    def paths_violate_constraint(self, constraint, paths):
        ...
        for i in range(len(paths)):
            ...
            if len(constraint['location']) == 1:  # vertex constraint
                ...
                if constraint['location'][0] == prev_location or constraint['location'][1] == current_location \
                        or constraint['location'] == [current_location, prev_location]:
                    result.append(i)
        return result
    ...
    def find_solution(self, disjoint, a_star_version):
        ...
        for i in range(self.num_of_agents):  # Find initial path for each agent
            ...
            astar = a_star_method(self.map, self.starts[i], self.goals[i], self.heuristics[i], i, root['constraints'])
        ...
        while len(self.open_list) > 0 and iteration < 1000:
            ...
            for constraint in constraints:
                ...
                ai = constraint['agent']
                astar = a_star_method(self.map, self.starts[ai], self.goals[ai], self.heuristics[ai], ai, new_node['constraints'])
                ...
                if path is not None:
                    ...
                    if constraint['positive']:
                        ...
                        for v in vol:
                            astar_v = a_star_method(self.map, self.starts[v], self.goals[v], self.heuristics[v], v, new_node['constraints'])
