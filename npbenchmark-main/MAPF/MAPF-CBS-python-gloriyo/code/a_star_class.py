...
class A_Star(object):
    ...
    # [{'agent': 3, 'location': [(2, 0)], 'timestep': 2, 'positive': False, 'meta_agent': {3}},
    # {'agent': 3, 'location': [(2, 1), (3, 1)], 'timestep': 4, 'positive': False, 'meta_agent': {3}},
    # {'agent': 1, 'location': [(3, 4), (3, 3)], 'timestep': 7, 'positive': False, 'meta_agent': {1}},
    ...
    def __init__(self, input_map, input_starts, input_goals, input_heuristics, agents, input_constraints):
        ...
        constraints - list of dict constraints generated by a CBS splitter; dict = {agent,location,timestep,positive}
        """
    ...
    def generate_child_nodes(self, current_node):
        ...
        for dirs in ma_dirs:
            ...
            for i, a in enumerate(self.agents):
                ...
                elif self.get_constraint_violated(current_node['location'][i], next_loc, current_node['timestep'] + 1, self.constraint_table[i],
                                                  self.agents[i]):
                    invalid_move = True
                if invalid_move:
                    break
    ...
    def low_level_search(self):
        ...
        for i, a in enumerate(self.agents):
            if root['location'][i] == self.goals[i]:
                ...
                if root['timestep'] <= self.max_constraints[i]:
                    if not self.future_constraint_violated(root['location'][i], root['timestep'], self.max_constraints[i],
        ...
        self.push_node(root)
        self.closed_list[(tuple(root['location']), root['timestep'])] = [root]
        ...
        while len(self.open_list) > 0:
            ...
            for child in children:
                ...
                if (tuple(child['location']), child['timestep']) in self.closed_list:
                    existing = self.closed_list[(tuple(child['location']), child['timestep'])]
                    ...
                    if (child['g_val'] + child['h_val'] < existing['g_val'] + existing['h_val']) and (
                        ...
                        self.closed_list[(tuple(child['location']), child['timestep'])] = child
